...
int main(int argc, char **argv)
{
   ...
   int lpos[channel_count];
    ...
    int optimisations = SAPR_OPTIMISATIONS_NONE;
    
    prog_name = argv[0];
    int opt;
    
    while(-1 != (opt = getopt(argc, argv, "hqvo:l:m:b:826extsf:")))
    {
        switch(opt)
        {
            case '2':
    ...
    if (optimisations > SAPR_OPTIMISATIONS_ALL)
    	cmd_error("optimisation level can only go between 0 and 7");
    ...
    fread(header_line, 1, 256, input_file);
    
    if(strstr(header_line, "SAP") != NULL)
    {
    	//fprintf(stderr, "%s\n", header_line);
    	fprintf(stderr,"SAPR: Skipping SAP Header\n");
    	
	for(int i=0; i<256; i++)
	{
		if(strstr(&header_line[i], "\n\r") != NULL)
			continue;
	...
	//fprintf(stderr, "SAPR: Offset by %d bytes\n", (int)pos);
    }
    
    // Set actual channel count
    if(strstr(header_line, "STEREO") != NULL)
    {
    	fprintf(stderr,"SAPR: Stereo Mode detected\n");
    	stereo = 1;
    }
    ...
    for(sz = 0;  1 == fread(buf, channel_count, 1, input_file) && sz < (128*1024); sz++ )
    {
        ...
        for (int i = 0; i < channel_count; i++) { data[i][sz] = buf[i]; }
